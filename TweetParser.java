...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        boolean columnsCheck = !(csvLine == null);

        if (columnsCheck) {
            String[] extractFile = csvLine.split(",", 0);
            if ((csvColumn >= extractFile.length) || (csvColumn < 0)) {
                return null;
            } else {
                return (extractFile [csvColumn]);
            }
        } else {
            return null;
        }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        FileLineIterator checkFile = new FileLineIterator(br);
        LinkedList<String> newSentences = new LinkedList<>();

        while (checkFile.hasNext()) {
            String findSentences = extractColumn(checkFile.next(), tweetColumn);
            if (findSentences != null) {
                newSentences.add(findSentences);
            }
        }
        return newSentences;
    }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        List<String> newSentences = new LinkedList<>();

        String[] split = sentence.split(" ");
        for (int i = 0; i < split.length; i++) {
            String findSentences = split[i];
            if (cleanWord(findSentences) != null) {
                newSentences.add(cleanWord(findSentences));
            }
        }
        return newSentences;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        String cleanSentences = removeURLs(tweet);
        List<String> splitSentences = tweetSplit(cleanSentences);
        List<List<String>> newSentences = new LinkedList<>();

        for (int i = 0; i < splitSentences.size(); i++) {
            String findSentences = splitSentences.get(i);
            List<String> parseClean = parseAndCleanSentence(findSentences);
            if (parseClean.size() > 0) {
                newSentences.add(parseClean);
            }
        }
        return newSentences;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<List<String>> dataProcess = new LinkedList<>();
        List<String> newSentences = csvDataToTweets(br, tweetColumn);

        for (int i = 0; i < newSentences.size(); i++) {
            String findSentence = newSentences.get(i);
            dataProcess.addAll(parseAndCleanTweet(findSentence));
        }
        return dataProcess;
    }
}