...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ProbabilityDistribution<String> probDistribution = new ProbabilityDistribution<>();
        ...
        if (this.chain.containsKey(first)) {
            ...
            probDistribution.record(second);
            this.chain.put(first, probDistribution);
        }
    }
    ...
    public void train(Iterator<String> sentence) {
        ...
        if (sentence != null) {
            if (sentence.hasNext()) {
                String nextSentence = sentence.next();
                this.startWords.record(nextSentence);

                while (sentence.hasNext()) {
                    String nextNextSentence = sentence.next();
                    this.addBigram(nextSentence, nextNextSentence);
                    nextSentence = nextNextSentence;
                }

                if (nextSentence.equals("")) {
                    return;
                } else {
                    this.addBigram(nextSentence, END_TOKEN);
                }
            }
        } else {
            throw new IllegalArgumentException("Error: This is a null sentence");
        }
    }
    ...
    public boolean hasNext() {
        boolean checkHas = !(this.next.equals(END_TOKEN));
        return checkHas;
    }
    ...
    @Override
    public String next() {
        String nextChain = this.next;
        if (nextChain.equals(END_TOKEN)) {
            throw new NoSuchElementException("Error: There are zero words");
        }

        boolean keyContain = !this.chain.containsKey(nextChain);
        if (keyContain) {
            this.next = END_TOKEN;
        } else {
            this.next = this.chain.get(nextChain).pick(ng);
        }
        return nextChain;
    }
