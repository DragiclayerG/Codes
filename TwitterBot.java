...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        List<List<String>> dataProcess = TweetParser.csvDataToTrainingData(br, tweetColumn);
        mc = new MarkovChain(ng);
        this.ng = ng;

        for (int i = 0; i < dataProcess.size(); i++) {
            List<String> transferringData = dataProcess.get(i);
            mc.train((transferringData.iterator()));
        }
    }
    ...
    public void writeStringsToFile(
        ...
        BufferedWriter bw;
        try {
            FileWriter dataWrite = new FileWriter(file, append);
            bw = new BufferedWriter(dataWrite);

            for (int i = 0; i < stringsToWrite.size(); i++) {
                String dataProcess = stringsToWrite.get(i);
                bw.write(dataProcess);
                bw.newLine();
            }
            bw.close();
        } catch (IOException failFile) {
            return;
        }
    }
    ...
    public String generateTweet(int numWords) {
        StringBuilder dataTweet = new StringBuilder();
        int wordsIterator = 1;
        this.mc.reset();

        if (numWords < 0) {
            throw new IllegalArgumentException("Error: There are negative inputs");
        }
        ...
        dataTweet.append(mc.next());
        while (numWords > wordsIterator) {
            while (mc.hasNext() && (wordsIterator < numWords)) {
                dataTweet.append(" ");
                dataTweet.append(mc.next());
                wordsIterator++;
            }
            dataTweet.append(this.randomPunctuation());
            mc.reset();
        }
        if (isPunctuated(dataTweet.toString())) {
            return dataTweet.toString();
        } else {
            dataTweet.append(this.randomPunctuation());
            return dataTweet.toString();
        }
